...
const GameCanvas = () => {
  ...
  const bricks = useRef(generateBricks());
    ...
    const draw = () => {
      ...
      if (ballY.current + ballRadius > canvas.height) {
        if (lives > 1) {
          ...
          ballY.current = paddleY - ballRadius - 1; // Place ball just above the paddle
          ballDY.current = -4;
          ballDX.current = 4;
          ballDY.current = -4;
        ...
        row.forEach((brick) => {
          if (brick.active) {
            if (
              ...
              // Check win condition
              const allCleared = bricks.current.every((row) =>
                row.every((b) => !b.active),
              );
              if (allCleared) {
                setGameWin(true);
                cancelAnimationFrame(animationFrameId);
    ...
    <>
      ...
      <button
        onClick={() => {
          ...
          ballDX.current = 4;
          ballDY.current = -4;
          paddleX.current = (800 - paddleWidth) / 2;
